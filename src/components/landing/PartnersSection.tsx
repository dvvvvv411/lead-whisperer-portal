...
const PartnersSection = () => {
  ...
  return <div className="flex flex-col items-center py-10">
      ...
      <motion.div className={`grid ${isMobile ? 'grid-cols-3 grid-rows-2' : 'grid-cols-2 md:grid-cols-3 lg:grid-cols-6'} gap-6 max-w-5xl mx-auto`} initial={{
      opacity: 0
    }} whileInView={{
      opacity: 1
    }} viewport={{
      ...
      delayChildren: 0.2
        ...
        {exchanges.map((exchange, index) => <ExchangeCard key={exchange.id} name={exchange.name} logo={exchange.logo} websiteUrl={exchange.websiteUrl} index={index} />)}
      </motion.div>
    </div>;
};

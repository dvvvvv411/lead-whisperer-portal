...
serve(async (req) => {
  ...
  try {
    ...
    for (const chatId of chatIds) {
      ...
      console.log(`Sending lead notification to Telegram chat ID: ${chatId}`);
    ...
    if (!allSuccess) {
      console.error('Some lead notifications failed to send:', results.filter(r => !r.success));
    } else {
      console.log('Lead notifications sent successfully to all chat IDs');
    }
    ...
    return new Response(
      JSON.stringify({ 
        success: true, 
        message: 'Lead notifications sent to Telegram',
        results: results
      }),
      { 
